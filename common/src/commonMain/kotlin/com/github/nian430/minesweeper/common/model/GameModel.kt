...
import com.github.nian430.minesweeper.common.db.DatabaseHelper
import com.github.nian430.minesweeper.common.platform.Difficulty
...
class GameModel(initialConfig: Config = Difficulty.values().first(), sqlDriver: SqlDriver) {

    private val _config = MutableStateFlow(initialConfig)
    val config get() = _config.asStateFlow()

    private val _progress = MutableStateFlow<Progress>(Progress.Pending)
    ...
    fun interact(row: Int, column: Int, action: BlockAction) {
        queue {
            ...
            if (isFirstDig) {
                ...
                _progress.value = Progress.OnGoing(System.currentTimeMillis())
            }
                ...
                when (progress) {
                    is Progress.OnGoing -> {
                        ...
                        if (isWin) {
                            ...
                            val previousTimeSpend = previousHighScore?.timeInMillis?.milliseconds ?: Duration.INFINITE
                        ...
                        Progress.Finish(isWin, timeSpend, highScoreTimeSpend ?: Duration.ZERO)
                    }
                    else -> progress
                }
            }
    ...
    private fun generateBlocks(firstRevealRow: Int, firstRevealColumn: Int, config: Config): List<List<Content>> {
    ...
    private fun initGame() {
        val currentConfig = _config.value
        _progress.value = Progress.Pending
